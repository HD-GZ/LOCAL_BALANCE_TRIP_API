...
public class PublicDataClient {
    ...
    private static final String QUOTA_EXCEEDED_REASON_CODE = "22";
    ...
    public JsonNode get(String baseUrl, String path, UnaryOperator<UriBuilder> customizer) {
        try {
            ...
            if (quotaExceeded(root)) {
                log.error("공공데이터 API 일일 한도 초과: path={}", path);
                throw BusinessException.of(ErrorCode.TOUR_API_QUOTA_EXCEEDED);
            }
    ...
    private boolean quotaExceeded(JsonNode root) {
        String reasonCode = root.path("OpenAPI_ServiceResponse")
            .path("cmmMsgHeader")
            .path("returnReasonCode")
            .asText();
        return QUOTA_EXCEEDED_REASON_CODE.equals(reasonCode);
    }

    public void rethrowIfQuotaExceeded(Exception e) {
        if (e instanceof BusinessException businessException
            && businessException.getErrorCode() == ErrorCode.TOUR_API_QUOTA_EXCEEDED) {
